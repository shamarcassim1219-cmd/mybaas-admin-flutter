...
class _UserDetailScreenState extends State<UserDetailScreen> {
  ...
  Future<void> _toggleDeviceBan() async {
    final deviceId = _user!.lastDeviceId;
    if (deviceId.isEmpty) return;

    final willBan = !_user!.deviceBanned;
    final reasonController = TextEditingController();

    final confirmed = await showDialog<bool>(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(willBan ? 'Ban Device' : 'Unban Device'),
        content: willBan
            ? Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('This blocks the device from logging in or registering on ANY account, not just this one.'),
            ...
            : const Text('Allow this device to log in and register again?'),
        actions: [
          TextButton(onPressed: () => Navigator.of(context).pop(false), child: const Text('Cancel')),
          TextButton(onPressed: () => Navigator.of(context).pop(true), child: Text(willBan ? 'Ban' : 'Unban')),
        ],
      ),
    );
    if (confirmed != true) return;
    ...
    try {
      if (willBan) {
        await AdminDeviceService.instance.ban(deviceId, reason: reasonController.text.trim());
      } else {
        await AdminDeviceService.instance.unban(deviceId);
      }
      _changed = true;
      await _load();
    } on ApiException catch (e) {
              ...
              : _user == null
                  ...
                  : ListView(
                      ...
                      children: [
                        ...
                        Text(
                          '${_user!.role.isNotEmpty ? _user!.role[0].toUpperCase() + _user!.role.substring(1) : 'Unknown'} account',
                        ...
                        if (_user!.lastDeviceId.isNotEmpty) ...[
                          ...
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: _user!.deviceBanned ? AppColors.dangerSoft : AppColors.surface,
                              borderRadius: BorderRadius.circular(AppSpacing.radiusMd),
                              border: Border.all(color: _user!.deviceBanned ? AppColors.danger.withValues(alpha: 0.3) : AppColors.border),
                            ),
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                ...
                                SizedBox(
                                  ...
                                  child: OutlinedButton(
                                    onPressed: _acting ? null : _toggleDeviceBan,
                                    style: OutlinedButton.styleFrom(
                                      foregroundColor: _user!.deviceBanned ? AppColors.success : AppColors.danger,
                                    ),
                                    child: Text(_user!.deviceBanned ? 'Unban This Device' : 'Ban This Device'),
                                  ),
                                ),
                              ],
                            ),
